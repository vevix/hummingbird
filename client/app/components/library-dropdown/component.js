...
export default Component.extend({
  ...
  updateTask: task(function *(status) {
    ...
    if (entry === undefined && get(this, 'entryIsLoaded') === true) {
      yield get(this, 'tasks.create')(status.key);
    } else if (entry !== undefined) {
      if (status.key === REMOVE_KEY) {
        yield get(this, 'tasks.delete')();
      } else {
        yield get(this, 'tasks.update')(status.key);
      }
    }
  }).drop(),
  ...
  init() {
    ...
    assert('`{{library-dropdown}}` requires a `tasks` hash param',
      get(this, 'tasks') !== undefined);
